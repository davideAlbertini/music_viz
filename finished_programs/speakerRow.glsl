...
vec2 loudspeakerDist(vec3 p, float cone_disp){
    ...
    //loudspeaker bulb
    vec3 p_bulb = p + vec3(0., 0., 0.25 + 0.03*cone_disp);
...
// using the union operator
    ...
    // plane 
    float planeDist = fPlane(p, vec3(0., 0.25, 0.), 1.);
    float planeID = 2.0;
    vec2 plane = vec2(planeDist, planeID);
    
    float spacing = delta/2.;
    float z = -0.6;
    float y = 2.9;
    
    vec2 loudspeaker1 = loudspeakerDist(p + vec3(spacing,y,z), cone_disp);
    vec2 loudspeaker2 = loudspeakerDist(p + vec3(-spacing,y,z), cone_disp);
    
    vec2 res = fOpUnion(plane, loudspeaker1);
    res = fOpUnion(res, loudspeaker2);
    
    int num_loudspeakers_each_side = 5;
    for(int j=1; j<num_loudspeakers_each_side; j++){
        res = fOpUnion(res, loudspeakerDist( p + vec3( spacing + float(j) * delta, y, z), cone_disp) );
        res = fOpUnion(res, loudspeakerDist( p + vec3(-spacing - float(j) * delta, y, z), cone_disp) );
    }
    ...
    vec3 p_box = p;
    p_box.z -=3.5;
    float gScreenDist = fBox(p_box, vec3(10., 10., 0.1));
    float gScreenID = 3.0;
    vec2 gScreen = vec2(gScreenDist, gScreenID);
    
    res = fOpUnion(res, gScreen);
...
void render(inout vec3 col, in vec2 uv){
    // sets ray origin
    vec3 ro = vec3(0., -0.5, -6.0);
    // sets ray directionS -> points towards xy plane 
    vec3 rd = normalize(vec3(uv+vec2(0.,0.4), FOV));
